...
try {
    ...
    if(_filter != null && _filter.equals("today")){
        filter = LocalDate.now().format(dateFormatter);
    }else if(_filter != null && _filter.equals("tomorrow")){
        filter = LocalDate.now().plusDays(1).format(dateFormatter);

    }else if(_filter != null && _filter.equals("dayaftertomorrow")){
        filter = LocalDate.now().plusDays(2).format(dateFormatter);
    }else {
        filter = LocalDate.now().format(dateFormatter);
    }
    ...
    if (Helper.StringPatternCheck(filter)){
