...
public class Helper {
    ...
    public void insertRandomNumToTxt(String randNum, String startDateTimeStr) {
        if (!checkRandomNumIsHere(randNum)) {
            try {
                FileWriter myWriter = new FileWriter("rndNums.txt", true);
    ...
    public static String getDayOfWeek(String inputText) {
        // Gün isimlerine karşılık gelen değerleri içeren bir Map oluştur
        Map<String, String> dayValues = new HashMap<>();
        dayValues.put("pazar", "0");
        dayValues.put("pazartesi", "1");
        dayValues.put("salı", "2");
        dayValues.put("çarşamba", "3");
        ...
        // Girdi metni küçük harfe dönüştürülüyor ve değer atanıyor
        String dayValue = dayValues.get(inputText.toLowerCase());

        // Eğer geçerli bir gün değeri alınamazsa "Bilinmeyen Gün" döndür
        if (dayValue == null) {
            return "7";
        }
